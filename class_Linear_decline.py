...
class Fit_Linear_Decline( object ):
    ...
    def _get_chi2minuit(self, a,b,t_peak):
        '''
        a
        b
        ...
        model_    = bounded_linear_decline(self.t_fit,a,b,t_peak)
    ...
    def set_minuit(self, guess, boundaries=None, fixed=None, step_sizes = None, print_level=0):
        ...
        a,b,t_peak   = guess
        self._paramname = "a,b,t_peak".split(",")
    ...
    def error_propag(self, min_interp = None, max_interp = None, nump = 500 , returnop = False ) :
        ...
        self.fit_mag_res , self.fit_mag_cov_res  = propagate(lambda param: bounded_linear_decline(self.t_res, *param), self.minuit.values, self.minuit.covariance)
    ...
    def error_via_parametric_bootstrap(self, min_interp = None, max_interp = None, nump = 1000 , returnop = False ) :
        ...
        self.fit_mag_res = bounded_linear_decline(self.t_res, *self.minuit.values[:])

        rng   = np.random.default_rng(1)
        par_b = rng.multivariate_normal(self.minuit.values, self.minuit.covariance, size=1000)

        y_b       = [bounded_linear_decline(self.t_res, *param) for param in par_b]
    ...
    def plot_fit(self ,add_plot = False, **kwargs):
        ...
        if add_plot == None:
            plt.figure()
            plt.errorbar(self.table['tfromexplo_zc'], self.table['mag'], self.table['emag'], fmt = 'o', ms=3.5 , color = self._filter_name.get(self.filter), **kwargs )
            # plt.plot(x_ , bounded_linear_decline(x_, *_params ), ls = '--' , color = _fitcolor )

            plt.plot(t_ , fit, ls = '--' , color = 'black' )
            plt.fill_between(t_, fit - e_fit, fit + e_fit,color = 'grey', alpha=0.5)

            
            # plt.plot(x_[1] , bounded_linear_decline(x_, *_params)[1], color = 'white',label= chi_fit_string )
            ...
            add_plot.plot(x_ ,bounded_linear_decline(x_, *_params), ls = '--'  , color = _fitcolor )

            
            add_plot.plot(x_[1] , bounded_linear_decline(x_,*_params)[1], color = 'white',label= chi_fit_string )
    ...
    def plot_fit_mag_normalised(self, add_plot = False):
        ...
        if self._peak_mag is not None: 
            ...
            if add_plot == None:
                ...
                plt.plot(x_ , bounded_linear_decline(x_, *_params )-self._peak_mag, ls = '--' , color = _fitcolor )
                ...
                plt.plot(x_[1] , bounded_linear_decline(x_, *_params)[1]-self._peak_mag, color = 'white',label= chi_fit_string )
                ...
                add_plot.plot(x_ , bounded_linear_decline(x_, *_params )-self._peak_mag, ls = '--'  , color = _fitcolor )
    ...
    def get_redchi_2_fit(self, param):
        ...
        ''' 
        #(a,b,t_peak)= param 
        model_  = bounded_linear_decline(self.t_fit,*param)
        pull_   = (( self.f_fit - model_ )/ self.e_f_fit )**2
        chi_sq  = np.sum(pull_)
        n_param = len(param)
