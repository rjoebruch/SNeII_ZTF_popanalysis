...
### We are using only the sample for which we can measure the decline ie the Silver sample, selected visually
standard_SNII = table_infants[table_infants['Silver_sample']=='Y']  

### When will work on the golden sample, need only the red band. 
## The decline will be an interactive code, need to plot the interpolation and then do the decline measurement 
...
for candiname in standard_SNII['name']:
    ...
    if detec_g is not None:
        ...
        interp_amag_g = interpolate_fullLC_with_errors(detec_g['tfromexplo_zc'], detec_g['absmag'],
                                                        detec_g['e_absmag'], low_bg, upp_bg, plot=False)
        smoothed_lc_Test = sg_perpart(interp_amag_g,peak_T=peak_g['pday']) # later change it to also get the errors and plot the errors 
        plt.figure()
